"""Validate a labelled CIC 6-class spare validation CSV with the saved artifact.

Examples
--------
    python resnet/validate_cic_holdout.py

    python resnet/validate_cic_holdout.py \
        --csv data/cic_public_6class_spare_validation.csv \
        --model-dir artifacts/resnet_cic_public
"""
...
DEFAULT_DATASET_NAME = "cic_public_6class_spare_validation"
...
def _find_model_dir(project_root: str, explicit: str | None) -> str:
    ...
    path = os.path.join(project_root, "artifacts", "resnet_cic_public")
    ...
    raise FileNotFoundError(f"Model directory not found: {path}. Use --model-dir to specify explicitly.")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Validate a labelled CIC 6-class spare validation CSV with a saved CIC model artifact.")
    parser.add_argument(
        "--csv",
        default=os.path.join(root, "data", f"{DEFAULT_DATASET_NAME}.csv"),
        help="Labelled CIC spare validation CSV.",
    )
    parser.add_argument(
        "--model-dir",
        default=None,
        help="Model artifact directory. Defaults to artifacts/resnet_cic_public when available.",
    )
    parser.add_argument("--dataset-name", default=DEFAULT_DATASET_NAME, help="Name used in output files.")

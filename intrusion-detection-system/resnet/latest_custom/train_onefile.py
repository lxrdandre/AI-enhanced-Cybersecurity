...
holdout is disabled by default; enable it explicitly when you want an untouched
post-training check. SMOTE is enabled in auto mode and only affects the training
split when class imbalance remains. Exact-row deduplication is disabled by
default because repeated Zeek flows are traffic volume signal, especially for
DoS, scanning, and password attacks.
"""
...
def main() -> None:
    ...
    metadata = {
        "script": "resnet/latest_custom/train_onefile.py",
        "dataset_name": "zeek_edge_crossval_6class",
        "data_csv": os.path.abspath(args.csv),

...
def _build_alerts(
    metrics: Dict[str, Any] | None, data_dir: Path
) -> List[Dict[str, Any]]:
    ...
    # Offline device alerts (aligned with web app)
    for router_id in anomalies:
        status = _heartbeat_status(data_dir, router_id)
        if status == "offline":
            alerts.append(
                {
...
async def dashboard() -> Dict[str, Any]:
    ...
    alerts = _build_alerts(metrics, data_dir)
    ...
    service = {
        ...
        "uptime": float(profile.get("uptime", 0.0)) / 100.0,
        "speed_consistency": float(profile.get("speed_consistency", 0.0)) / 100.0,
    }
